...
def ncsn_conv1x1(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0, rngs=None):
    """
    1x1 convolution with PyTorch initialization. Same as NCSNv1/v2.
    return a nnx.Module
    """
    ...
    kernel_shape = (1, 1) + (in_planes, out_planes)
    bias_init = lambda key, shape: kernel_init(key, kernel_shape)[0, 0, 0, :]
    return nn.Conv(
...
# def ddpm_conv1x1(x, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0, name=None):
...
def ddpm_conv1x1(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0, rngs=None):
    """1x1 convolution with DDPM initialization."""
    bias_init = jnn.initializers.zeros
    return nn.Conv(
        in_features=in_planes,
...
def ncsn_conv3x3(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0, rngs=None):
    """3x3 convolution with PyTorch initialization. Same as NCSNv1/NCSNv2."""
    init_scale = 1e-10 if init_scale == 0 else init_scale
    kernel_init = jnn.initializers.variance_scaling(
        1 / 3 * init_scale, "fan_in", "uniform"
    )
    kernel_shape = (3, 3) + (in_planes, out_planes)
    bias_init = lambda key, shape: kernel_init(key, kernel_shape)[0, 0, 0, :]
    return nn.Conv(
        in_features=in_planes,
        out_features=out_planes,
...
def ddpm_conv3x3(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0, rngs=None):
...
class CRPBlock(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(n_stages):
            self.convs.append(ncsn_conv3x3(features, features, stride=1, bias=False, rngs=rngs))
        if maxpool:
...
class NIN(nn.Module):
    """
    similar to nn.Dense, but only conduct on the last dimension (channel)
    ...
    def __init__(self,
        ...
        self.num_units = in_dim if num_units is None else num_units
